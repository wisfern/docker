...
if [ ! $PASSWORDHASH ]; then
    export PASSWORDHASH='$1$UsonvAZ1$rLjg3JaoHKhlzz5R5EoIb0'
fi

addgroup --gid 999 ubuntu && \
useradd -m -u 999 -s /bin/zsh -g ubuntu devis
echo "devis:$PASSWORDHASH" | /usr/sbin/chpasswd -e
echo "devis    ALL=(ALL) ALL" >> /etc/sudoers
unset PASSWORDHASH

# devis user add zsh configure
su - devis
cd /opt && git clone https://github.com/wisfern/dotfiles.git
chmod -R o+w dotfiles
cd dotfiles
make zsh
exit
